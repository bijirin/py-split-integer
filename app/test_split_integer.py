from app.split_integer import split_integer


def test_num_of_parts_should_be_equal_to_parts() -> None:
    assert len(split_integer(10, 3)) == 3
    assert len(split_integer(10, 4)) == 4
    assert len(split_integer(10, 5)) == 5
    assert len(split_integer(10, 6)) == 6


def test_max_min_parts_should_differ_by_at_most_one() -> None:
    parts = split_integer(10, 3)
    assert max(parts) - min(parts) <= 1

    parts = split_integer(10, 4)
    assert max(parts) - min(parts) <= 1

    parts = split_integer(10, 5)
    assert max(parts) - min(parts) <= 1

    parts = split_integer(10, 6)
    assert max(parts) - min(parts) <= 1


def test_sum_of_the_parts_should_be_equal_to_value() -> None:
    assert sum(split_integer(10, 3)) == 10
    assert sum(split_integer(10, 4)) == 10
    assert sum(split_integer(10, 5)) == 10
    assert sum(split_integer(10, 6)) == 10


def test_should_split_into_equal_parts_when_value_divisible_by_parts() -> None:
    assert split_integer(10, 2) == [5, 5]
    assert split_integer(12, 3) == [4, 4, 4]
    assert split_integer(15, 5) == [3, 3, 3, 3, 3]


def test_should_return_part_equals_to_value_when_split_into_one_part() -> None:
    assert split_integer(10, 1) == [10]
    assert split_integer(12, 1) == [12]
    assert split_integer(15, 1) == [15]


def test_parts_should_be_sorted_when_they_are_not_equal() -> None:
    assert split_integer(10, 3) == [3, 3, 4]
    assert split_integer(13, 4) == [3, 3, 3, 4]
    assert split_integer(16, 5) == [3, 3, 3, 3, 4]


def test_should_add_zeros_when_value_is_less_than_number_of_parts() -> None:
    assert split_integer(2, 5) == [0, 0, 0, 1, 1]
    assert split_integer(3, 6) == [0, 0, 0, 1, 1, 1]
    assert split_integer(1, 4) == [0, 0, 0, 1]
